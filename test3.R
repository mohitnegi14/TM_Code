...
# com2 <- com[pis_encoded == '100882278871']
# com3 <- com[year == 1986,]
...
###############################################################################################
# Turns out, the 68 batches is not enough. It is still too big for my computer. I will further
# subdivide it by 20, so we will have 68x20 = 1360 batches. 

# load(glue('{raw}/relevant_pis_codes_batches.rda'))
# 
# for(batch_number in 1:68) {
#   
#   # Load in the data.
#   message(glue('WORKING ON BATCH {batch_number} OF 68.'))
#   message(glue('-------------------------------------'))
#   
#   ## STEP 1: LOAD IN ALL THE COHORTS AND REFERENCES DATA AND THE WORKPLACES DATA FOR THE BATCH.
#   workplaces_df <- data.table::fread(glue('{harddrive}/workplaces_batch_{batch_number}.csv'),
#                                      colClasses = c('character', 'character', 'numeric', 'character'))
#   
...
#   relevant_pis_codes_subbatches <- split(relevant_pis_codes_batches[[batch_number]], ceiling(seq_along(relevant_pis_codes_batches[[batch_number]]) / size_of_one_batch))
#   
#   sub_batch_divider <- function(sub_batch_pis) {
#     
#     message(glue('WORKING ON SUB-BATCH {sub_batch_pis} OF 20 IN BATCH {batch_number} OF 68.'))
#     message(glue('-------------------------------------'))
#     
#     # Keep the workplaces data for the current sub-batch only.
#     workplaces_df_subbatch <- workplaces_df[pis_encoded %in% relevant_pis_codes_subbatches[[sub_batch_pis]],]
#     
#     # Write it as a csv.
#     data.table::fwrite(workplaces_df_subbatch, glue('{harddrive}/subbatches/workplaces_batch_{batch_number}.{sub_batch_pis}.csv'))
#     
#     rm(workplaces_df_subbatch)
#     gc()
#     
#     return(NULL)
#     
#   }
#   
#   lapply(c(1:length(relevant_pis_codes_subbatches)), sub_batch_divider)
#   
#   rm(workplaces_df)
#   gc()
#   
...
batch_numbers <- list.files(path = glue('{harddrive}/subbatches/')) %>% str_sub(18, -5)

# First define the function that we will apply to each workplaces batch.
compute_connections <- function(batch_param) {
  
  ticker <- which(batch_numbers == batch_param)
  
  message(glue('WORKING ON BATCH {ticker} OF 1356.'))
  message(glue('-------------------------------------'))
  
  ## STEP 1: LOAD IN THE WORKPLACES DATA FOR THE BATCH.
  workplaces_df <- data.table::fread(glue('{harddrive}/subbatches/workplaces_batch_{batch_param}.csv'),
                                     colClasses = c('character', 'character', 'numeric', 'character'))
  
  batch_param_main <- str_split_1(batch_param, pattern = '\\.')[1]
  batch_param_sub <- str_split_1(batch_param, pattern = '\\.')[2]
  
  size_of_one_batch <- 5000
  relevant_pis_codes_subbatches <- split(relevant_pis_codes_batches[[batch_param_main]], ceiling(seq_along(relevant_pis_codes_batches[[batch_param_main]]) / size_of_one_batch))
  
  pis_in_batch <- relevant_pis_codes_subbatches[[batch_param_sub <- str_split_1(batch_param, pattern = '\\.')[2]]]
  
  references_in_batch <- references[pis_encoded %in% pis_in_batch,]
  
  rm(pis_in_batch)
  rm(relevant_pis_codes_subbatches)
  rm(batch_param_main)
  rm(batch_param_sub)
  
  message(glue('STEP 1 COMPLETED FOR BATCH {ticker}/1356.'))
  message(glue('--------------------------------------------'))
  ...
  message(glue('STEP 2 COMPLETED FOR BATCH {ticker}/1356.'))
  message(glue('--------------------------------------------'))
  ...
  joined_workplaces <- references_workplaces[new_entrants_workplaces, on = .(workplace, establishment, year_workplace, monthyear_workplace, reference_year == cohort), .(workplace, monthyear_workplace, i.pis_encoded, establishment, cohort, pis_encoded), nomatch = 0, allow.cartesian = T][(pis_encoded != i.pis_encoded) | is.na(pis_encoded),]
  ...
  message(glue('STEP 3 COMPLETED FOR BATCH {ticker}/1356.'))
  message(glue('--------------------------------------------'))
  ...
  data.table::fwrite(collapsed_data, file = glue('{harddrive}/collapsed/collapsed_data_{batch_param}.csv'))
  
  message(glue('STEP 4 COMPLETED FOR BATCH {ticker}/1356.'))
  message(glue('--------------------------------------------'))
...
lapply(batch_numbers[185:1356], FUN = compute_connections)
