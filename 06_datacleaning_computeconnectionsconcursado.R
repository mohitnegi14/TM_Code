...
# 1. An additional match which looks at only 1 year ago matches and splits by concursado or not.
...
entrants_workplaces_concursado <- data.table::fread(glue('{harddrive}/intermediate/public_sector_workers/workplaces_entrants_concursado.csv'),
...
compute_connections_fn <- function(batch_param) {
  ...
  tryCatch(
    withTimeout(
      {
        ...
        ## So now we have 2 datasets - entrants_workplaces_concursado and references_workplaces.
        ...
        joined_workplaces <- references_workplaces[entrants_workplaces_concursado, .(establishment, i.pis_encoded, cohort, pis_encoded, monthyear_workplace, concursado, public, one_year_prior), nomatch = 0, allow.cartesian = T][(pis_encoded != i.pis_encoded) | is.na(pis_encoded),]
        ...
        write(batch_param, file = glue('{harddrive}/intermediate/connections/completed_connections_match_concursado.txt'), append = T, sep = '\n')
    ...
    error = function(e) {
      write(batch_param, file = glue('{harddrive}/intermediate/connections/errors_connections_match_concursado.txt'), append = T, sep = '\n')
...
compute_connections_subbatches_fn <- function(error_batch_param) {
  ...
  tryCatch(
    withTimeout(
      {
        ...
        write(error_batch_param, file = glue('{harddrive}/intermediate/connections/completed_connections_match_concursado.txt'), append = T, sep = '\n')
    ...
    error = function(e) {
      write(error_batch_param, file = glue('{harddrive}/intermediate/connections/errors_connections_match_concursado.txt'), append = T, sep = '\n')
...
final_data_connections <- connected_entrants[entrants_concursado, on = .(i.pis_encoded = pis_encoded, cohort, establishment)]
final_data_connections_one_year <- connected_entrants_one_year[entrants_concursado, on = .(i.pis_encoded = pis_encoded, cohort, establishment)]
