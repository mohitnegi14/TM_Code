...
lapply(periods$monthyear[127:132], FUN = redo_workplaces_fn)
...
for(batch_param in batch_numbers[883:1158]) {
  ...
  error = function(e) {
    write(batch_param, file = glue('{box}/intermediate/connections2/errors_connections_match_concursado.txt'), append = T, sep = '\n')
...
# Now add in the entrants who have 0 placebo connections. Do this by using the entrants dataset
# and then matching with the new values we created.
